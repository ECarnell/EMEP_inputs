...
# 2. For the remaining years & pollutants, take the regional emissions in Lat Long and;
       ...
       # iii) For SNAP 1: ONLY point sources go into A_PublicPower as this is treated with 200m injection. All other into B_Industry.

combined.data <- combine.all.region.data(yr.poll.subset = yr.poll.subset, mapping.yr = mapping.yr, class = class, region = region) # c. 2.5 mins per year & pollutant

# 3. Using the newly classified GNFR data, place into a netcdf (either existing or new)
input.to.netcdf(combined.data = combined.data, years = unique(yr.poll.subset[,years]), pollutants = unique(yr.poll.subset[,pollutants]), yr.poll.subset = yr.poll.subset, mapping.yr = mapping.yr, class = class, region = region)
